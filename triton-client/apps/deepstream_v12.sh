...
#
# ARCHITECTURE CHANGE:
#   v11 used nvstreamdemux to SPLIT streams across branches (each camera
#   processed by only ONE model). This broke nvdsmetamux (designed for
#   tee pattern) and nvdslogger (showed half the streams).
#
#   v12 adopts the NVIDIA reference architecture from
#   github.com/NVIDIA-AI-IOT/deepstream_parallel_inference_app:
#
#     nvmultiurisrcbin → tee ──→ demux0 → branch0_mux → pgie0 → sgie0 ──→ metamux
#                             └→ demux1 → branch1_mux → pgie1 → sgie1 ──↗
#
#   Every camera is processed by EVERY model. metamux correctly merges
#   metadata from matching source_id + frame_num pairs. nvdslogger sees
#   all streams.
#
# KEY DESIGN DECISIONS:
#   - tee zero-copies batched buffers to all branches (GPU memory shared)
#   - Each branch has its OWN nvstreamdemux (pads pre-created)
#   - branch_mux sink pads use source_id directly (sink_5 for camera 5)
#     so nvstreammux preserves the original source_id — no restore probe
#   - Deferred linking (g_idle_add) from v11 preserved for deadlock safety
#   - Dynamic add/remove via REST API still works
#
# RETAINED FROM v11:
#   - Deferred link pattern (g_idle_add) prevents deadlock
#   - BranchFpsTracker for per-source FPS at branch outputs
#   - DemuxPadTracker for slot consumption monitoring
#
...
echo "=========================================="
echo "2. Writing C++ App (v12 - Tee-Based Parallel Inference)..."
echo "=========================================="
 ...
 *   v11 used a single nvstreamdemux to SPLIT streams to branches (each camera
 *   processed by one model). This broke metamux and nvdslogger.
 *
 * v12 uses the NVIDIA reference tee pattern:
 *   nvmultiurisrcbin → tee → demux0 → branch0_mux → pgie0 → ... → metamux
 *                           → demux1 → branch1_mux → pgie1 → ... → metamux
 *
 *   Every camera goes through EVERY model. metamux merges matching frames.
 *
 * KEY DESIGN:
 *   - tee zero-copies batched buffer to N branches
 *   - Each branch has its own nvstreamdemux (pads pre-created before PLAYING)
 *   - branch_mux uses sink_<source_id> naming so source_id is preserved
 *   - Deferred linking via g_idle_add prevents deadlock (from v11)
 *   - Deleted source's demux pad is consumed forever (never reused)
 */
